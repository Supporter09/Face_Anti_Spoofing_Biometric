    ...
    <main className="result-page">
      <section className={`session-result-card ${isLive ? 'live' : 'spoof'}`}>
        ...
        <dl className="result-stats">
          <div>
            <dt>Passive score</dt>
            <dd>{verdict.passive_avg.toFixed(2)} ≥ {T_PASSIVE}</dd>
          </div>
          <div>
            <dt>Challenge</dt>
          ...
          {!isLive ? (
            <>
              <div>
                <dt>Quay trái (rel.)</dt>
                <dd>{verdict.challenge_eval.max_yaw_left?.toFixed(1) ?? '—'}° / -{YAW_TARGET}°</dd>
              </div>
              <div>
                <dt>Quay phải (rel.)</dt>
                <dd>{verdict.challenge_eval.max_yaw_right?.toFixed(1) ?? '—'}° / +{YAW_TARGET}°</dd>
              </div>
            </>
          ) : null}
        </dl>
