...
if "%LIVENESS_MODEL_PATH%"=="" (
    set "MODEL_PATH=%ROOT_DIR%\Kaggle_Outputs\context_mobilenetv2_224\mobilenetv2_context_scripted.pt"
) else (
...
if "%LIVENESS_SPOOF_THRESHOLD%"=="" (
    set "LIVENESS_SPOOF_THRESHOLD=0.3"
)

if "%LIVENESS_LIVE_THRESHOLD%"=="" (
    set "LIVENESS_LIVE_THRESHOLD=0.5"
)

REM Debug mode
REM Example:
REM set LIVENESS_DEBUG_DIR=C:\temp\fas_debug
REM run_backend_demo.bat

if not "%LIVENESS_DEBUG_DIR%"=="" (
